...
//      COLLECTION);
  ...
  Debt from(Row row) {
    ...
    return Debt.builder()
        ...
        .previousPaymentAmountCurrency(row.getString("previous_payment_amount_currency"))
        .build();
  }
  ...
  public Stmt stmtInsert(long receiptId, Collection<Debt> debts) {
    ...
    for (Debt debt : debts) {
      ...
      values[i++] = Value.text(debt.previousPaymentAmountCurrency());
    }
  ...
  public Uni<Integer> update(Debt debt) {
    ...
    return tursoWsService.executeQuery(
            Stmt.stmt(UPDATE,
                ...
                Value.text(debt.previousPaymentAmountCurrency()),
