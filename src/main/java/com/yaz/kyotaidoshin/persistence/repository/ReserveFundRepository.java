...
public class ReserveFundRepository {
  ...
  private static final String INSERT_BULK = """
      INSERT INTO %s (building_id, name, fund, expense, pay, active, type, expense_type, add_to_expenses) VALUES %s
      """;
  ...
  public Uni<Integer> insertBulk(List<ReserveFund> reserveFunds) {
    final var values = reserveFunds.stream().flatMap(reserveFund -> {
      return Stream.of(
          Value.text(reserveFund.buildingId()), Value.text(reserveFund.name()), Value.number(reserveFund.fund()),
          Value.number(reserveFund.expense()), Value.number(reserveFund.pay()), Value.bool(reserveFund.active()),
          Value.enumV(reserveFund.type()), Value.enumV(reserveFund.expenseType()),
          Value.bool(reserveFund.addToExpenses())
      );
    }).toArray(Value[]::new);

    final var aptValuesParam = SqlUtil.valuesParams(9, reserveFunds.size());

    return tursoWsService.executeQuery(Stmt.stmt(INSERT_BULK.formatted(COLLECTION, aptValuesParam), values))
        .map(executeResp -> executeResp.result().rowCount());
  }
}
