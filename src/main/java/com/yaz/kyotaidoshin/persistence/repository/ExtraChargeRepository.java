...
import com.yaz.kyotaidoshin.persistence.model.ExtraCharge.Apt;
...
public class ExtraChargeRepository {
  ...
  public Uni<InsertResult> insert(ExtraCharge extraCharge, Collection<String> apartments) {
    ...
    final var insertStmt = Stmt.stmt(INSERT, Value.text(extraCharge.parentReference()),
        ...
        Value.number(extraCharge.amount()), Value.text(extraCharge.currency()), Value.bool(extraCharge.active()));

    return tursoWsService.selectOne(insertStmt, row -> row.getLong("id"))
        .map(opt -> opt.orElseThrow(() -> new IllegalStateException("Insert failed " + insertStmt)))
        .flatMap(id -> {
          ...
          final var aptValuesParam = SqlUtil.valuesParams(4, apartments.size());
...
//              .collect(Collectors.joining(","));
  ...
  public Uni<Integer> update(ExtraCharge extraCharge) {
    ...
    stmts[i++] = Stmt.stmt(UPDATE,
        ...
        NamedArg.text("currency", extraCharge.currency()),
  ...
  public Uni<Integer> insertBulk(List<ExtraCharge> extraCharges) {
    ...
    return Multi.createFrom().iterable(extraCharges)
        .onItem()
        .transformToUni(extraCharge -> {
          final var strings = extraCharge.apartments().stream().map(Apt::number).toList();
          return insert(extraCharge, strings);
        })
        .merge()
        .map(InsertResult::count)
        .collect()
